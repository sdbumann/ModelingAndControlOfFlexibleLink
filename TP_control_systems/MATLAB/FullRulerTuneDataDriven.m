...
Sarray_ = zeros(4, 6);
Tarray_ = zeros(4, 6);
scheduling_par_array_ = [70,50,30,15];

[~, Rarray_(1, :), Sarray_(1, :), Tarray_(1, :)] = DataDriven(G_70mm, false);
[~, Rarray_(2, :), Sarray_(2, :), Tarray_(2, :)] = DataDriven(G_50mm, false);
[~, Rarray_(3, :), Sarray_(3, :), Tarray_(3, :)] = DataDriven(G_30mm, false);
[~, Rarray_(4, :), Sarray_(4, :), Tarray_(4, :)] = DataDriven(G_15mm, false);
%%
[scheduling_par_array, Rarray] = matrix_interpolation(scheduling_par_array_, Rarray_, 80);
[~, Sarray] = matrix_interpolation(scheduling_par_array_, Sarray_, 80);
[~, Tarray] = matrix_interpolation(scheduling_par_array_, Tarray_, 80);
% 
if 0 
    figure
    bode(G_70mm, G_50mm, G_30mm, G_15mm)
    legend('G70mm', 'G50mm', 'G30mm', 'G15mm')
    figure()
